...
library(ggrepel)
source("consumption-contracts.R", local = knitr::knit_global())
...
validate_task_credits(consumption, credit_tasks)
...
person_consumption <- consumption |>
  ...
  summarise(
    ...
    total_credits = sum(Copilot_credits_consumed),
...
person_base <- people |>
  ...
  mutate(
    ...
    ReasoningProfile = case_when(
      !positive_consumer ~ "No consumption",
      is.na(total_tokens) | total_tokens < 10000 |
        !is.finite(reasoning_share) | !is.finite(reasoning_cut) ~
          "Insufficient token volume",
      reasoning_share >= reasoning_cut ~ "Reasoning-intensive",
      TRUE ~ "Standard-cost mix"
    ),
    ConsumptionProfile = case_when(
      ReasoningProfile == "Insufficient token volume" ~ "Insufficient token volume",
...
profile_summary <- person_wow |>
  filter(ReasoningProfile %in% c("Reasoning-intensive", "Standard-cost mix")) |>
  ...
  disclose_breakdown(MIN_GROUP_N)
...
if (!nrow(profile_grid)) {
  cat("Reasoning-profile breakdown withheld: insufficient reportable population.")
} else {
ggplot(
  profile_grid,
...
| Standard-cost mix | At least 10,000 tokens and below the reasoning-intensive threshold | Less expensive observed token mix |
| Insufficient token volume | Positive credits but insufficient observed tokens to classify | Unclassified; excluded from measured-profile comparisons |
...
Unclassified consumers remain in population and credit-volume denominators.
They are never silently included in the standard-cost group.

```{r profile-coverage}
profile_coverage <- person_base |>
  count(ReasoningProfile, name = "People") |>
  disclose_breakdown(MIN_GROUP_N)
if (!nrow(profile_coverage)) {
  cat("Classification counts withheld: at least one positive cell is below the disclosure floor.")
} else {
  kable(profile_coverage, caption = "Classification coverage, including unclassified consumers.")
}
```
...
if (!nrow(profile_summary)) {
  cat("Reasoning-profile outcomes withheld: insufficient reportable population.")
} else {
profile_summary |>
...
band_totals <- person_base |>
  group_by(CreditBand) |>
  summarise(TotalCredits = sum(total_credits), .groups = "drop")

task_mix <- credit_tasks |>
  left_join(select(person_base, PersonId, CreditBand), by = "PersonId") |>
  filter(CreditBand != "No consumption", credits > 0) |>
  group_by(CreditBand, TaskType) |>
  summarise(Credits = sum(credits), People = n_distinct(PersonId),
            .groups = "drop") |>
  left_join(band_totals, by = "CreditBand", relationship = "many-to-one") |>
  mutate(Share = Credits / TotalCredits) |>
  disclose_breakdown(MIN_GROUP_N)

if (!nrow(task_mix)) {
  cat("Task breakdown withheld: at least one positive cell is below the disclosure floor.")
} else {
task_mix |>
  ...
  labs(
    ...
    subtitle = "Same person/window weekly-credit denominator; the entire breakdown is withheld if any positive cell is too small",
...
if (!nrow(segment_credit)) {
  cat("Credit-volume breakdown withheld: at least one positive cell is below the disclosure floor. No residual shares are published.")
} else {
...
| Privacy | Withhold the entire segment/task cross-tab if any positive cell is below the approved minimum; never publish reconstructable residuals |
| Task allocation | Validate unique person/task/window keys and reconcile to each person's full-window weekly credits before plotting; fail on mismatch |
...
```text
templates/sample-code/utility-r/_data/consumption/
  consumption-query/
  person-query/
  reference/
```
...
The task allocation is generated locally from the same-window weekly credit
totals by `generate-demo-data.R`; it is not an independently measured task feed.
See `_data/README.md` for provenance, seeds and the inclusive window contract.
...
7. Reconcile task-feed coverage, dates and person totals before interpreting task
   shares. A mismatch stops this example; do not rescale the feed to conceal it.

This synthetic demonstration is not a disclosure certification for tenant data.
In particular, concentration curves and distribution plots require an approved
release policy beyond subgroup-count checks before any real-data publication.
