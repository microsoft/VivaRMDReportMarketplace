...
# Aggregate repeated Person Query rows before splitting so each person belongs
# to exactly one of the training or test sets.
raw_data <- raw_data |>
  group_by(PersonId) |>
  summarise(
    across(where(is.numeric) & !any_of("performance"), ~mean(.x, na.rm = TRUE)),
