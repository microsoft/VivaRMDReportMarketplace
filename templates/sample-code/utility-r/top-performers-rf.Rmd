...
> **Note on the data.** This demo dataset is de-identified: each person is represented only by a hashed `PersonId` alongside a handful of aggregate collaboration metrics, and it contains no names, email addresses, or organisational attributes. It is included purely to illustrate the modelling workflow, so please substitute your own Person Query when applying this to real data.
...
The `createDataPartition()` function from **caret** makes it easy to split the data into training and testing datasets, where the `p` argument controls the proportion of the data that goes into the training set.

The checked-in demo dataset contains multiple rows per person, so we partition the unique people rather than the individual rows. If we split rows directly, records belonging to the same person could appear in both sets, which leaks information across the split and makes the reported test metrics overly optimistic. Once the split is made, `PersonId` is no longer needed and is dropped from both data frames.

```{r}
# Set randomisation split
set.seed(123)

# One row per person, used as the unit of the split
person_data <-
  clean_data %>%
  group_by(PersonId) %>%
  summarise(perform_cat = first(perform_cat), .groups = "drop")

# Split people into training and testing sets
trainIndex <- caret::createDataPartition(
  y = person_data$perform_cat, # outcome
  p = 0.7, # percentage goes into training
  list = FALSE # do not return result as list
  )

train_ids <- person_data$PersonId[trainIndex]

train_df <- clean_data %>% filter(PersonId %in% train_ids) %>% select(-PersonId)
test_df <- clean_data %>% filter(!(PersonId %in% train_ids)) %>% select(-PersonId)
```
