# Local synthetic task-allocation fixture. Source this file to use the generator without writes.
...
write_demo_data <- function(data_dir) {
  weekly <- read.csv(file.path(data_dir, "consumption-query", "consumption-weekly.csv"),
                     stringsAsFactors = FALSE)
  write.csv(generate_credit_tasks(weekly),
            file.path(data_dir, "consumption-query", "consumption-task-types.csv"),
            row.names = FALSE, fileEncoding = "UTF-8")
}

if (sys.nframe() == 0L) {
  script <- sub("^--file=", "", grep("^--file=", commandArgs(), value = TRUE))
  write_demo_data(file.path(dirname(normalizePath(script)), "_data", "consumption"))
}
